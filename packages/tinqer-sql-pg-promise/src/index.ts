...
export function query<TParams, TResult>(
  ...
  // TEMPORARILY DISABLED - parseQuery needs to be reimplemented for new architecture
  throw new Error("query() function needs to be reimplemented with new converter architecture");
 ...
 */
export function toSql<T>(_queryable: Queryable<T> | OrderedQueryable<T> | TerminalQuery<T>): {
  text: string;
  parameters: Record<string, unknown>;
} {
  // TEMPORARILY DISABLED - parseQuery needs to be reimplemented for new architecture
  throw new Error("toSql() function needs to be reimplemented with new converter architecture");
...
>(
  _db: PgDatabase,
  _queryBuilder: (params: TParams) => TQuery,
  _params: TParams,
): Promise<
  TQuery extends Queryable<infer T>
    ...
    : TQuery extends OrderedQueryable<infer T>
      ...
      : TQuery extends TerminalQuery<infer T>
        ...
        : never
> {
  // TEMPORARILY DISABLED - needs to be reimplemented for new architecture
  throw new Error("execute() function needs to be reimplemented with new converter architecture");
  ...
  // // Handle different terminal operations - COMMENTED OUT TEMPORARILY
  // const operationType = ""; // Temporary to avoid errors
